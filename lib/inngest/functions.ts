...
import { updateAndSaveSentiment } from "@/lib/actions/market.actions";
...
export const generateMarketSentiment = inngest.createFunction(
  { id: "generate-market-sentiment" },
  { cron: "0 */2 * * *" }, // ทุก2ชั่วโมง
  async ({ step }) => {
    
    const result = await step.run("generate-and-save", async () => {
      // เรียกฟังก์ชันกลางตัวเดิมเลย
      const data = await updateAndSaveSentiment();
      return data;
    });
